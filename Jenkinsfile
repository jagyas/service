pipeline {
  ...
  stages {
    stage("Build") {
      steps {
        container("kaniko") {
          sh "/kaniko/executor:arm64 --context `pwd` --destination vfarcic/jenkins-demo:latest --destination ${REGISTRY_USER}/${PROJECT}:${env.BRANCH_NAME.toLowerCase()}-${BUILD_NUMBER}"
        }
      }
    }
    stage("Test") {
